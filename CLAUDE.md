...
- **Dependency advisories**: this app is a static, fully client-side bundle. Only `dependencies` reach `dist/`; everything under `devDependencies` (eslint, vite, typescript and their transitives) runs on a dev machine or in CI over trusted input, so DoS/ReDoS advisories there are noise, not exposure. The real gate is `npm audit --omit=dev --audit-level=high`, run in CI before every deploy. Do not chase dev-only advisories one by one — let the weekly grouped Dependabot PR (`.github/dependabot.yml`) carry them.
